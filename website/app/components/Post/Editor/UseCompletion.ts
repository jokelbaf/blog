...
import { Completion } from './CompletionExtension';
import type { CompletionStorage } from './CompletionExtension';

type CompletionMode = 'continue' | 'fix' | 'extend' | 'reduce' | 'simplify' | 'summarize';
...
export function useEditorCompletion(editorRef: Ref<{ editor: Editor | undefined } | null | undefined>, options: UseEditorCompletionOptions = {}) {
	...
	watch(completion, (newCompletion, oldCompletion) => {
		...
		if (storage?.visible) {
			...
			if (delta) {
				if (['fix', 'simplify'].includes(mode.value)) {
					delta = delta.replace(/[\r\n]+/g, ' ').replace(/\s{2,}/g, ' ');
				}
